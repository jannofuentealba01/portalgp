 ...
 */
function msp2EstadosLocalesBuildPaginationItems(int $paginaActual, int $totalPaginas): array
{
    if ($totalPaginas <= 1) {
        return [];
    }
...
try {
    $tablaExiste = msp2TableExists($conn, 'msp_estado_locales');
    if (!$tablaExiste) {
        $loadError = 'La tabla `msp_estado_locales` no existe todavía. Ejecuta `msp/msp_a1.sql` antes de continuar.';
    }
} catch (PDOException $exception) {
    $tablaExiste = false;
    $loadError = 'No fue posible validar la estructura base de estados de locales.';
}
...
if ($tablaExiste) {
    try {
        ...
        $countStmt = $conn->prepare("SELECT COUNT(*) FROM dbo.msp_estado_locales WHERE $whereClause");
        ...
        $stmt = $conn->prepare(
            "SELECT id_estado_local, desc_estado
...
if ($tablaExiste) {
    $paginationItems = msp2EstadosLocalesBuildPaginationItems($paginaActual, $totalPaginas);
}

$queryBase = $_GET;
unset($queryBase['pagina']);

function buildMsp2EstadosLocalesQuery(array $base, array $override = []): string
{
...
<main class="gp-main d-flex align-items-center justify-content-center p-4">
    <div class="box-container-wide">
        ...
        gpRenderSectionHeader([
            ...
            'title' => 'Estados de Locales',
            'description' => 'Catálogo base para el estado operativo de cada local.',
            'back_url' => msp2Url('catalogo_menu.php'),
            'back_label' => 'Volver a catálogos',
            'help_text' => 'Administra el catálogo de estados operativos de locales para mantener la clasificación funcional en MSP.',
            'help_aria_label' => 'Información de la sección Estados de Locales',
        ]);
        ?>
        ...
        <?php else: ?>
            ...
            gpRenderCrudTable([
                                    ...
                                    'items' => [
                                        [
                                            ...
                                            'attrs' => [
                                                ...
                                                'data-id' => (string) ((int) ($estado['id_estado_local'] ?? 0)),
                ...
                'pagination' => [
                    ...
                    'aria_label' => 'Paginación de estados de locales',
                    'build_url' => static fn (int $page): string => buildMsp2EstadosLocalesQuery($queryBase, ['pagina' => $page]),
                ],
            ]);
            ?>
        <?php endif; ?>
...
<div class="modal fade" id="modalCrearEstado" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" method="post" action="<?php echo msp2Escape(msp2Url('estados_locales/guardar.php')); ?>">
...
<div class="modal fade" id="modalEditarEstado" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" method="post" action="<?php echo msp2Escape(msp2Url('estados_locales/guardar.php')); ?>">
            ...
            <div class="modal-body">
                <input type="hidden" name="id_estado_local" id="edit_id_estado_local">
                <div class="mb-3">
...
(() => {
    document.querySelectorAll('.js-edit-estado').forEach((button) => {
        button.addEventListener('click', () => {
            document.getElementById('edit_id_estado_local').value = button.dataset.id || '';

...
function ceFetchReportDataset(PDO $conn, string $periodoConsumoYm): array
{
    $procesoStmt = $conn->prepare(
        "SELECT TOP (1)
         ...
         WHERE UPPER(ts.codigo_servicio) = 'LUZ'
           AND EXISTS (
                SELECT 1
    ...
    if ($idProceso <= 0) {
        throw new RuntimeException('No existe proceso de LUZ para el período de consumo seleccionado.');
    }
    $valorKwProceso = round((float) ($proceso['valor_kwh'] ?? 0), 6);
    if ($valorKwProceso <= 0) {
        throw new RuntimeException('Debes guardar un VALOR_KW válido en el paso de LUZ.');
    }
    ...
    if ($dbRows === []) {
        throw new RuntimeException('No hay lecturas de LUZ registradas para este proceso.');
    }
    ...
    foreach ($dbRows as $row) {
        ...
        $aPagar = $montoTotalCobro !== null
            ...
            : round($consumido * $valorKwProceso, 2);
...
$servicio = strtoupper(trim((string) ($_GET['servicio'] ?? 'LUZ')));
...
if ($servicio !== 'LUZ') {
    ...
    echo 'Este reporte es solo para LUZ (electricidad).';
    exit();
}
...
$title = 'CONSUMO ELECTRICIDAD ' . ceMonthYearLabel($periodoYm);
...
try {
    ...
    $headers = [
        'Local',
        'med. ant. KW (' . $fechaAnteriorHdr . ')',
        'med. act. KW (' . $fechaActualHdr . ')',
        'TOTAL CONSUMIDO',
        '$ KWH',
        'A PAGAR',
    ];
    $nextPeriodoYm = ceNextPeriodoYm($periodoYm);
    $nextPeriodoPdfTitle = 'CONSUMO ELECTRICIDAD ' . (
    ...
    if ($format === 'debug') {
        ...
        echo '<!doctype html><html lang="es"><head><meta charset="UTF-8"><title>Debug agrupación electricidad</title>'
        ...
        echo '<h2>Debug agrupación consumo eléctrico</h2>';
        echo '<div><strong>Servicio:</strong> ELECTRICIDAD (LUZ)</div>';
    ...
    if ($format === 'xlsx') {
        ...
        $sheet->setTitle('Consumo electrico');
        ...
        $sheet->getStyle('A3:F3')->getFill()->getStartColor()->setRGB('FF8989');
        ...
        foreach ($rows as $row) {
            ...
            if ($useGrayGroup) {
                ...
                $sheet->getStyle('A' . $rowIndex . ':F' . $rowIndex)->getFill()->getStartColor()->setRGB('BFC8A2');
            }
            $rowIndex++;
        }
        ...
        if (abs($totalAPagar) < 0.000001) {
            ...
            $sheet->setCellValue('F' . $rowIndex, $totalAPagar);
        }
        $sheet->getStyle('A' . $rowIndex . ':F' . $rowIndex)->getFont()->setBold(true);
        $sheet->getStyle('A' . $rowIndex . ':F' . $rowIndex)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID);
        $sheet->getStyle('A' . $rowIndex . ':F' . $rowIndex)->getFill()->getStartColor()->setRGB('F8E5E2');
        $sheet->getStyle('F' . $rowIndex)->getNumberFormat()->setFormatCode('#,##0');

        if ($addendumRows !== []) {
            $sheet->getStyle('B4:F' . max(4, $rowIndex - 1))->getNumberFormat()->setFormatCode('#,##0');
            $sheet->getStyle('A3:F' . $rowIndex)->getBorders()->getAllBorders()
        ...
        $tmpFile = tempnam(sys_get_temp_dir(), 'msp2_consumo_luz_');
        if ($tmpFile === false) {
        ...
        $filename = 'consumo_electrico_' . ceFileMonthYearLabel($periodoYm) . '.xlsx';
    ...
    foreach ($rows as $row) {
        ...
        $bodyRows .= '<tr' . $rowClass . '>'
            ...
            . '<td class="num">' . msp2Escape(ceFmtNum($row['valor_kw'] ?? 0, 0)) . '</td>'
            . '<td class="num">' . msp2Escape(ceFmtAPagar($row['a_pagar'] ?? 0, 0)) . '</td>'
            . '</tr>';
    }
    $bodyRows .= '<tr>'
        . '<td colspan="5" style="font-weight:bold;background:#f8e5e2;">TOTAL A PAGAR</td>'
    ...
    $html = '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
        ...
        . $bodyRows
        . '</tbody></table>'
        . '<div class="meta">A PAGAR = TOTAL CONSUMIDO x $ KWH</div>'
        . ($addendumBodyRows !== ''
    ...
    $filename = 'consumo_electrico_' . ceFileMonthYearLabel($periodoYm) . '.pdf';

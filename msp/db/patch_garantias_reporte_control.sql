...
WITH movimientos AS (
 ...
 SELECT g.id_garantia,COALESCE(cl.id_contrato_arriendo,g.id_contrato_arriendo) id_contrato_arriendo,g.id_contrato_local,
  ...
  CAST(ISNULL(r.recibido,0)-ISNULL(m.aplicado_d,0)-ISNULL(m.aplicado_r,0)-ISNULL(m.devuelto,0)+ISNULL(m.ajuste_pos,0)-ISNULL(m.ajuste_neg,0) AS DECIMAL(18,2)) saldo_operativo,
 ...
 SELECT b.*,
  ...
  CAST(CASE WHEN b.disponible_modelo<b.saldo_operativo THEN CASE WHEN b.disponible_modelo>0 THEN b.disponible_modelo ELSE 0 END ELSE CASE WHEN b.saldo_operativo>0 THEN b.saldo_operativo ELSE 0 END END AS DECIMAL(18,2)) disponible
 FROM base b
)
SELECT m.id_garantia,m.id_contrato_arriendo,m.id_contrato_local,m.id_local,m.id_tienda,m.id_arrendatario,m.nombre_locatario,m.rut,m.nombre_comercial,m.cdo_local,m.desc_local,m.fecha_constitucion,m.estado_garantia,
 ...
 CASE WHEN m.pactado=0 THEN N'SIN_MONTO' WHEN m.recibido=0 THEN N'NO_RECIBIDA' WHEN m.recibido<m.pactado THEN N'RECEPCION_PARCIAL' WHEN m.recibido>m.pactado THEN N'RECEPCION_EXCEDIDA' WHEN m.reservado>0 THEN N'SALDO_RESERVADO' WHEN m.saldo_operativo<0 OR m.disponible_modelo<0 THEN N'INCONSISTENCIA_SALDO' ELSE N'OK' END alerta_codigo,
 CASE WHEN m.pactado=0 THEN N'Garantía configurada sin monto pactado.' WHEN m.recibido=0 THEN N'Existe monto pactado sin recepción confirmada.' WHEN m.recibido<m.pactado THEN N'La recepción de garantía está incompleta.' WHEN m.recibido>m.pactado THEN N'El monto recibido supera lo pactado.' WHEN m.reservado>0 THEN N'Existe saldo reservado por cargos.' WHEN m.saldo_operativo<0 OR m.disponible_modelo<0 THEN N'El saldo neto de la garantía es negativo.' ELSE N'Sin alertas.' END alerta_descripcion,
 CASE WHEN m.saldo_operativo<0 OR m.disponible_modelo<0 OR m.recibido>m.pactado THEN 3 WHEN m.pactado>0 AND m.recibido<m.pactado THEN 2 WHEN m.pactado=0 OR m.reservado>0 THEN 1 ELSE 0 END alerta_nivel
FROM metricas m;
GO

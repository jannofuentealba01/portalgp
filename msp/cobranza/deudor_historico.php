...
try {
    ...
    $error = pgpPublicOrBusinessException($exception, 'msp.cobranza.deudor_historico', 'No fue posible cargar el detalle del deudor histórico.');
}
...
?>
<!doctype html>
<html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Deudor histórico | MSP</title><link rel="stylesheet" href="/portalgp/assets/vendor/bootstrap-5.3.0/css/bootstrap.min.css"><link rel="stylesheet" href="/portalgp/assets/vendor/bootstrap-icons-1.11.3/font/bootstrap-icons.css"><link rel="stylesheet" href="/portalgp/styles.css">
</head>
<body class="gp-layout bg-light gp-module-msp"><?php include dirname(__DIR__, 2) . '/templates/header.php'; ?><main class="gp-main"><div class="box-container-full dhd-shell mx-auto">
...
<details class="gp-context-help"><summary>Cómo interpretar el desglose</summary><p>Cada documento conserva su saldo pendiente real. Las líneas muestran los conceptos originales —arriendo, agua, luz, gas u otros—; un pago parcial no se distribuye artificialmente entre esas líneas.</p></details>
<div class="dhd-card mb-4"><div class="card-header bg-white"><strong>Documentos pendientes y sus conceptos</strong></div><div class="card-body py-2"><?php if ($documentos === []): ?><p class="text-muted mb-0">No hay documentos pendientes asociados al contrato.</p><?php else: foreach ($documentos as $documento): ?><details class="gp-row-detail dhd-documento px-3 py-2 my-2"><summary><span><strong><?php echo msp2Escape((string) ($documento['numero_documento'] ?: ('Documento #' . $documento['id_documento_cobro']))); ?></strong><span class="dhd-small ms-2">Período <?php echo msp2Escape(dhdFecha($documento['periodo_facturacion'])); ?> · Vence <?php echo msp2Escape(dhdFecha($documento['fecha_vencimiento'])); ?></span></span><span class="ms-auto ps-2"><span class="me-3">Total <?php echo msp2Escape(dhdMonto($documento['monto_total'])); ?></span><strong class="text-danger">Pendiente <?php echo msp2Escape(dhdMonto($documento['saldo_pendiente'])); ?></strong></span></summary><div class="pt-2"><div class="small fw-semibold mb-1">Conceptos originales</div><ul class="mb-0 small"><?php foreach ($detallesPorDocumento[(int) $documento['id_documento_cobro']] ?? [] as $detalle): ?><li><?php echo msp2Escape((string) ($detalle['descripcion_item'] ?: $detalle['nombre_item'] ?: 'Concepto sin descripción')); ?>: <strong><?php echo msp2Escape(dhdMonto($detalle['subtotal'])); ?></strong></li><?php endforeach; ?></ul></div></details><?php endforeach; endif; ?></div></div>
<div class="dhd-card mb-4"><div class="card-header bg-white"><strong>Cargos directos pendientes</strong></div><div class="table-responsive gp-table-shell"><table class="table table-bordered align-middle mb-0 gp-table-compact gp-table-mobile-cards msp-historical-charges-table"><thead class="table-light"><tr><th>Fecha / local</th><th>Concepto</th><th>Resumen financiero</th></tr></thead><tbody><?php if ($cargos === []): ?><tr><td colspan="3" class="text-center text-muted gp-table-empty-cell">No hay cargos directos pendientes.</td></tr><?php else: foreach ($cargos as $cargo): ?><tr><td data-gp-label="Fecha / local"><strong><?php echo msp2Escape(dhdFecha($cargo['fecha_cargo'])); ?></strong><div class="small text-muted">Local <?php echo msp2Escape((string) $cargo['cdo_local']); ?></div></td><td data-gp-label="Concepto" class="gp-cell-description"><?php echo msp2Escape((string) $cargo['descripcion_cargo']); ?></td><td data-gp-label="Resumen financiero" class="gp-financial-cell"><span class="gp-data-pair"><span>Original</span><strong><?php echo msp2Escape(dhdMonto($cargo['monto_cargo'])); ?></strong></span><span class="gp-data-pair"><span>Garantía aplicada</span><strong class="text-success"><?php echo msp2Escape(dhdMonto($cargo['monto_aplicado_garantia'])); ?></strong></span><span class="gp-data-pair gp-data-pair--total"><span>Saldo</span><strong class="text-danger"><?php echo msp2Escape(dhdMonto($cargo['saldo_pendiente'])); ?></strong></span></td></tr><?php endforeach; endif; ?></tbody></table></div></div>

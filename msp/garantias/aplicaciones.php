...
$returnTo = msp2GarantiaAplicacionSafeReturnTo($_GET['return_to'] ?? '');
$returnUrl = $returnTo !== '' ? $returnTo : 'garantias/index.php';
$returnLabel = 'Volver a Garantías';
...
} elseif (str_starts_with($returnTo, 'contratos/liquidacion_final.php?')) {
    $returnLabel = 'Volver a liquidación';
...
if ($idContratoFiltro > 0) {
    $selfParams['id_contrato_arriendo'] = $idContratoFiltro;
}
if ($returnTo !== '') {
    $selfParams['return_to'] = $returnTo;
}
$selfPath = 'garantias/aplicaciones.php' . ($selfParams !== [] ? '?' . http_build_query($selfParams) : '');
...
try {
    ...
    $error = pgpPublicOrBusinessException($exception, 'msp.garantias.aplicaciones', 'No fue posible cargar la aplicación de garantías.');
}
?>
<!doctype html>
...
<body class="gp-layout gp-module-msp bg-light"><?php include dirname(__DIR__,2).'/templates/header.php'; ?>
<main class="gp-main container-fluid py-3 px-lg-4 ga-page">
<div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3" data-gp-commandbar><div><p class="text-muted mb-1">MSP / Garantías</p><h1 class="h3 mb-0">Aplicar garantía a deuda</h1></div><a class="btn btn-outline-secondary btn-sm" href="<?php echo msp2Escape(msp2Url($returnUrl));?>"><i class="bi bi-arrow-left me-1"></i><?php echo msp2Escape($returnLabel); ?></a></div>
...
<div class="card shadow-sm mb-4"><div class="card-body"><form method="get" class="row g-2 align-items-end"><input type="hidden" name="id_contrato_arriendo" value="<?php echo $idContratoFiltro > 0 ? (int) $idContratoFiltro : ''; ?>"><?php if ($returnTo !== ''): ?><input type="hidden" name="return_to" value="<?php echo msp2Escape($returnTo); ?>"><?php endif; ?><div class="col-lg-10"><label class="form-label fw-semibold">Buscar garantía</label><input type="search" name="q" class="form-control" value="<?php echo msp2Escape($q);?>" placeholder="Arrendatario, RUT, contrato o local"><?php if ($idContratoFiltro > 0): ?><div class="form-text"><i class="bi bi-link-45deg me-1"></i>Contrato precargado: <strong>#<?php echo (int) $idContratoFiltro; ?></strong>. Los resultados quedan limitados a este contrato.</div><?php endif; ?></div><div class="col-lg-2 d-grid"><button class="btn btn-primary"><i class="bi bi-search me-1"></i>Buscar</button></div></form></div></div>
...
<div class="card shadow-sm"><div class="card-header fw-semibold">Cargos adicionales compatibles</div><div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0 ga-unified-table gp-table-compact gp-table-mobile-cards"><thead class="table-light"><tr><th>Arrendatario / local</th><th>Origen de deuda</th><th>Garantía / deuda</th><th>Operación / observaciones</th></tr></thead><tbody>
<?php if($cargos===[]):?><tr><td colspan="4" class="text-center text-muted py-4"><?php echo $q===''?'Busca un arrendatario para consultar sus cargos.':'No existen cargos adicionales pendientes compatibles con garantías recibidas.';?></td></tr><?php endif;?>
...
<details class="card shadow-sm mt-4"><summary class="card-header fw-semibold" style="cursor:pointer">Historial de operaciones (<?php echo count($historial); ?>)</summary><div class="table-responsive"><table class="table table-sm table-hover align-middle mb-0 ga-unified-table gp-table-compact gp-table-mobile-cards"><thead class="table-light"><tr><th>Arrendatario / local</th><th>Origen de deuda</th><th>Garantía / movimiento</th><th>Operación / observaciones</th></tr></thead><tbody><?php if($historial===[]):?><tr><td colspan="4" class="text-center text-muted py-3">Sin operaciones registradas.</td></tr><?php endif;?><?php foreach($historial as $h):?><tr><td><div class="fw-semibold"><?php echo msp2Escape((string)$h['nombre_locatario']);?></div><div class="small text-muted">Contrato #<?php echo (int)$h['id_contrato_arriendo'];?> · Local <?php echo msp2Escape((string)$h['cdo_local']);?></div></td><td><?php echo $h['id_documento_cobro']?'Documento #'.(int)$h['id_documento_cobro']:'Cargo #'.(int)$h['id_cargo_salida'];?> · <?php echo msp2Escape((string)$h['descripcion_cargo']);?></td><td><div class="fw-semibold"><?php echo msp2Escape((string)$h['nombre_movimiento']);?></div><div class="small text-muted"><?php echo $h['fondo_origen']==='R'?'Reservado':($h['fondo_origen']==='D'?'Disponible':'-');?> · <?php echo msp2Escape(substr((string)$h['fecha_movimiento'],0,10));?></div><div class="fw-semibold"><?php echo msp2Escape(gaMonto($h['monto_movimiento']));?></div></td><td><?php echo msp2Escape((string)($h['observaciones']??'-'));?></td></tr><?php endforeach;?></tbody></table></div></details>
</main><script<?= function_exists('pgpCspNonceAttribute') ? pgpCspNonceAttribute() : '' ?> src="/portalgp/assets/vendor/bootstrap-5.3.0/js/bootstrap.bundle.min.js"></script><script<?= function_exists('pgpCspNonceAttribute') ? pgpCspNonceAttribute() : '' ?>>(()=>{document.querySelectorAll('.form-operacion').forEach(f=>{const a=f.querySelector('.accion'),m=f.querySelector('.monto'),l=f.querySelector('.limite');const sync=()=>{let max=0;if(a.value==='RESERVAR')max=Number(a.dataset.reservar||0);else if(a.value==='APLICAR_DESDE_DISPONIBLE')max=Number(a.dataset.disponible||0);else max=Number(a.dataset.reservado||0);m.max=max>0?String(max):'';l.textContent=max>0?'Máximo para esta acción: $ '+max.toLocaleString('es-CL'):'';};a.addEventListener('change',sync);sync();});})();</script></body></html>

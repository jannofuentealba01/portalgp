...
require_once dirname(__DIR__).'/bootstrap.php';msp2RequireAccess('MSP Cobranza','escritura');
...
if(!$idGarantia||!$idCuenta||!$okMonto||$monto===null||(float)$monto<=0||!$date||$date->format('Y-m-d')!==$fecha||!in_array($medio,['EFECTIVO','TRANSFERENCIA'],true)||$beneficiario===''||$motivo===''||mb_strlen($beneficiario)>200||mb_strlen($motivo)>500||mb_strlen($obs)>500){msp2SetFlash('warning','Completa correctamente los datos y el motivo de autorización.');msp2Redirect(msp2WithPendingReturn('garantias/devoluciones.php?id_garantia='.(int)$idGarantia,$returnTo));}
if($medio==='TRANSFERENCIA'&&($banco===''||$cuenta===''||$ref==='')){msp2SetFlash('warning','La transferencia requiere banco, cuenta destino y referencia.');msp2Redirect(msp2WithPendingReturn('garantias/devoluciones.php?id_garantia='.(int)$idGarantia,$returnTo));}
try{
...
msp2Redirect(msp2WithPendingReturn('garantias/devoluciones.php?id_garantia='.(int)$idGarantia,$returnTo));

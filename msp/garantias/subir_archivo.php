...
require_once dirname(__DIR__).'/bootstrap.php';require_once __DIR__.'/archivo_helper.php';msp2RequireAccess('MSP Cobranza','escritura');
...
try{
 ...
 $tmp=(string)($file['tmp_name']??'');if($tmp===''||!is_uploaded_file($tmp))throw new RuntimeException('La carga temporal no es válida.');$finfo=new finfo(FILEINFO_MIME_TYPE);$mime=(string)$finfo->file($tmp);$allowed=['application/pdf'=>'pdf','image/jpeg'=>'jpg','image/png'=>'png'];if(!isset($allowed[$mime])||!msp2GarantiaArchivoValidateContent($tmp,$mime))throw new RuntimeException('El contenido no corresponde a un PDF, JPG o PNG válido.');
 ...
 $original=msp2GarantiaArchivoSafeName((string)($file['name']??'respaldo')).'.'.$allowed[$mime];$stored=bin2hex(random_bytes(16)).'.'.$allowed[$mime];$absolute=$dir.DIRECTORY_SEPARATOR.$stored;if(!move_uploaded_file($tmp,$absolute))throw new RuntimeException('No fue posible almacenar el archivo.');@chmod($absolute,0640);

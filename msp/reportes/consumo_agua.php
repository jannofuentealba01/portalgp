...
function reFetchConsumoDataset(PDO $conn, string $periodoConsumoYm): array
{
    ...
    $procesoStmt = $conn->prepare(
        "SELECT TOP (1)
         ...
         WHERE UPPER(ts.codigo_servicio) = 'AGUA'
           AND EXISTS (
                SELECT 1
    ...
    if ($idProceso <= 0) {
        throw new RuntimeException('No existe proceso de AGUA para el período de consumo seleccionado.');
    }
...
function reBuildTrendSeries(PDO $conn, string $periodoYm, int $limit = 6, int $maxLookback = 24): array
{
    ...
    while (count($labels) < $limit && $steps < $maxLookback) {
        ...
        try {
            ...
            // Mes sin proceso AGUA: se omite del gráfico.
        }
...
$reportePdfUrl = null;
if ($periodoValido) {
    $baseQuery = 'servicio=AGUA&periodo=' . urlencode($periodoYm) . '&anadido_siguiente=1';
    $reporteExcelUrl = msp2Url('cobros/reporte_consumo_agua.php?' . $baseQuery . '&format=xlsx');
    $reportePdfUrl = msp2Url('cobros/reporte_consumo_agua.php?' . $baseQuery . '&format=pdf');
}
...
if ($periodoValido) {
    try {
        ...
        $resumen = [
            ...
            'divisor' => $dataset['divisor'] ?? null,
            'tarifa_variable' => $dataset['tarifa_variable'] ?? null,
            'cargo_fijo' => $dataset['cargo_fijo'] ?? null,
...
<head>
    ...
    <title>MSP | Reporte Consumo Agua</title>
    ...
    <style>
        .agua-report-shell{max-width:1640px;width:100%;margin:0 auto;font-family:"Segoe UI","Helvetica Neue",Arial,sans-serif}
        .agua-report-header{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;gap:1rem;margin-bottom:.85rem}
        .agua-report-header h1{grid-column:2;grid-row:1;justify-self:center;margin:0;color:#003399;font-size:1.75rem;font-weight:600;line-height:1.2}
        .agua-report-back{grid-column:1;grid-row:1;justify-self:start}
        .agua-report-actions{grid-column:3;grid-row:1;justify-self:end;display:flex;flex-wrap:wrap;justify-content:flex-end;gap:.5rem}
        .agua-report-header .btn{min-height:34px;padding:.35rem .65rem;font-size:.825rem;line-height:1.2}
        .agua-report-filter{margin-bottom:.85rem}
        .agua-report-filter .form-label{margin-bottom:.25rem;font-size:.88rem}
        .agua-report-filter .btn{min-height:38px;padding-top:.38rem;padding-bottom:.38rem}
        .agua-report-kpi{height:100%;border:1px solid #dce5ef;border-radius:8px;background:#fff;padding:.65rem .75rem}
        .agua-report-kpi .small{font-size:.72rem}
        .agua-report-kpi .h4{color:#123f72;font-size:1.08rem;line-height:1.25}
        .agua-report-section{margin-top:.85rem;padding-top:.7rem;border-top:1px solid #dbe3ec}
        .agua-report-panel{height:100%;border:1px solid #dce5ef;border-radius:8px;background:#fff;padding:.65rem .75rem}
        .agua-report-panel h2,.agua-report-panel h3{margin-bottom:.45rem}
        .agua-report-panel .table>:not(caption)>*>*{padding:.38rem .45rem;font-size:.82rem}
        ...
        .agua-report-main-chart{height:320px;min-height:320px}
        @media(max-width:700px){.agua-report-header{display:flex;flex-direction:column;align-items:stretch}.agua-report-header h1{order:1;align-self:center;text-align:center}.agua-report-back{order:2}.agua-report-actions{order:3;justify-content:flex-start}.agua-report-actions .btn{flex:1 1 auto}.msp-report-chart-box,.agua-report-main-chart{height:240px;min-height:240px}}
    </style>
</head>
...
<main class="gp-main p-3 p-xl-4">
    <div class="agua-report-shell">
        ...
        <?php msp2RenderFlash($flash); ?>
                ...
                <form method="get" class="row g-2 align-items-end agua-report-filter" id="form_reporte_consumo_periodo">
                    <?php
                ...
                <?php else: ?>
                    ...
                    <?php else: ?>
                        <?php
                        $divisor = $resumen['divisor'];
                        $tarifaVariable = $resumen['tarifa_variable'];
                        $cargoFijo = $resumen['cargo_fijo'];
                        ?>
                        <div class="row g-2">
                            <div class="col-6 col-md-4 col-xl">
                                <div class="agua-report-kpi">
                                    <div class="small text-muted">Consumo total (m³)</div>
                            ...
                            <div class="col-6 col-md-4 col-xl">
                                <div class="agua-report-kpi">
                                    <div class="small text-muted">Divisor general</div>
                                    <div class="h4 mb-0"><?php echo msp2Escape($divisor !== null ? reFmtNum((float) $divisor, 2) : '--'); ?></div>
                                </div>
                            </div>
                            <div class="col-6 col-md-4 col-xl">
                                <div class="agua-report-kpi">
                                    <div class="small text-muted">Tarifa variable</div>
                                    <div class="h4 mb-0"><?php echo msp2Escape($tarifaVariable !== null ? reFmtMoney((float) $tarifaVariable) : '--'); ?></div>
                                </div>
                            </div>
                            <div class="col-6 col-md-4 col-xl">
                                <div class="agua-report-kpi">
                                    <div class="small text-muted">Cargo fijo</div>
                                    <div class="h4 mb-0"><?php echo msp2Escape($cargoFijo !== null ? reFmtMoney((float) $cargoFijo) : '--'); ?></div>
                                </div>
                            </div>
                        ...
                        <section class="agua-report-section">
                            <div class="agua-report-panel">
                                ...
                                <div class="msp-report-chart-box agua-report-main-chart">
                                    <canvas id="chartTendenciaLine"></canvas>
                                </div>
                            </div>
                        </section>

                        <div class="row g-2 agua-report-section">
                            <div class="col-12">
                                <div class="agua-report-panel">

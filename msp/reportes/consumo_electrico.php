...
function reFetchConsumoDataset(PDO $conn, string $periodoConsumoYm): array
{
    ...
    $valorKwh = is_numeric((string) ($proceso['valor_kwh'] ?? null))
        ? (float) $proceso['valor_kwh']
        : null;
    ...
    foreach ($dbRows as $row) {
        ...
        $montoUsado = $montoTotal !== null
            ...
            : (($valorKwh !== null && $valorKwh > 0) ? round($consumoUsado * $valorKwh, 2) : 0.0);
...
function reBuildTrendSeries(PDO $conn, string $periodoYm, int $limit = 6, int $maxLookback = 24): array
{
    ...
    while (count($labels) < $limit && $steps < $maxLookback) {
        ...
        try {
            ...
            // Mes sin proceso LUZ: se omite del gráfico.
        }
...
$reportePdfUrl = null;
if ($periodoValido) {
    $baseQuery = 'servicio=LUZ&periodo=' . urlencode($periodoYm) . '&anadido_siguiente=1';
    $reporteExcelUrl = msp2Url('cobros/reporte_consumo_electrico.php?' . $baseQuery . '&format=xlsx');
    $reportePdfUrl = msp2Url('cobros/reporte_consumo_electrico.php?' . $baseQuery . '&format=pdf');
}
...
if ($periodoValido) {
    try {
        ...
        $resumen = [
            ...
            'valor_kwh' => $dataset['valor_kwh'] ?? null,
...
<main class="gp-main p-3 p-xl-4">
    <div class="gas-report-shell">
        <header class="gas-report-header">
            ...
            <h1>Consumo eléctrico mensual</h1>
            <div class="gas-report-actions">
                <?php if ($periodoValido): ?>
                    <a href="<?php echo msp2Escape((string) $reporteExcelUrl); ?>" class="btn btn-success btn-sm"><i class="bi bi-file-earmark-spreadsheet me-1" aria-hidden="true"></i>Descargar Excel</a>
                    <a href="<?php echo msp2Escape((string) $reportePdfUrl); ?>" class="btn btn-outline-danger btn-sm" target="_blank" rel="noopener"><i class="bi bi-file-earmark-pdf me-1" aria-hidden="true"></i>Ver PDF</a>
                <?php endif; ?>
            </div>
        </header>
        ...
        <?php msp2RenderFlash($flash); ?>
                ...
                <?php else: ?>
                    ...
                    <?php else: ?>
                        <?php $valorKwh = $resumen['valor_kwh']; ?>
                        <div class="row g-2">
                            <div class="col-6 col-md-4 col-xl">
                                <div class="gas-report-kpi">
                                    <div class="small text-muted">Consumo total (kWh)</div>
                            ...
                            <div class="col-6 col-md-4 col-xl">
                                <div class="gas-report-kpi">
                                    <div class="small text-muted">Valor kWh</div>
                                    <div class="h4 mb-0"><?php echo msp2Escape($valorKwh !== null ? reFmtMoney((float) $valorKwh) : '--'); ?></div>
                                </div>
                            </div>
                    ...
                    tooltip: {
                        callbacks: {
                            label: (context) => {
                                ...
                                return `Consumo: ${new Intl.NumberFormat('es-CL', { maximumFractionDigits: 2 }).format(value)} kWh`;
                            },
                        },
                    },
                },

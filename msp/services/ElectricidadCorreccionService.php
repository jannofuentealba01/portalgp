...
/** Corrección controlada de lecturas eléctricas ya documentadas. */
final class ElectricidadCorreccionService
{
    public static function ejecutar(PDO $conn, array $corr, int $usuario): array
    {
        ...
        foreach (['msp_lecturas_medidores','msp_medidores','msp_contrato_locales','msp_contratos_arriendo',
                  'msp_procesos_cobro_servicio','msp_tipos_servicio','msp_proceso_cobro_luz',
                  'msp_cobros_servicios','msp_documentos_cobro','msp_documentos_cobro_detalle'] as $tabla) {
            if (!msp2TableExists($conn, $tabla)) {
                throw new RuntimeException('Falta la estructura requerida para corregir la lectura eléctrica.');
            }
        }
        ...
        try {
            $claim = $conn->prepare(
                "UPDATE dbo.msp_correcciones
                 SET estado_correccion=N'EJECUTANDO',estrategia_ejecucion=N'LECTURA_ELECTRICIDAD_CONTROLADA',
            ...
            if ((string) ($lectura['codigo_servicio'] ?? '') !== 'LUZ') {
                throw new RuntimeException('La corrección controlada implementada corresponde solamente a electricidad.');
            }
            if ((int) ($lectura['id_cobro_servicio'] ?? 0) <= 0) {
                throw new RuntimeException('La lectura no tiene un cobro eléctrico calculado. Vuelve a generar la etapa de electricidad.');
            }
            ...
            $tarifaActual = (float) ($lectura['valor_kwh'] ?? 0);
            if (!is_finite($tarifaActual) || $tarifaActual < 0) {
                throw new RuntimeException('La tarifa eléctrica del período no es válida.');
            }
            ...
            $montoNuevo = round($consumoNuevo * (float) ($lectura['valor_kwh'] ?? 0), 2);
            $afectados = [[
                'tipo' => 'actual',
                ...
                'valor_kwh' => (float) ($lectura['valor_kwh'] ?? 0),
            ]];
            ...
            if ($propagaSiguiente) {
                ...
                $tarifaSiguiente = (float) ($siguiente['valor_kwh'] ?? 0);
                if (!is_finite($tarifaSiguiente) || $tarifaSiguiente < 0) {
                    throw new RuntimeException('La tarifa eléctrica del período siguiente no es válida.');
                }
                ...
                $afectados[] = [
                    ...
                    'monto_nuevo' => round($consumoSiguiente * (float) ($siguiente['valor_kwh'] ?? 0), 2),
                    'valor_kwh' => (float) ($siguiente['valor_kwh'] ?? 0),
                ];
            }
            ...
            $resultado = [
                'estrategia' => 'LECTURA_ELECTRICIDAD_CONTROLADA',
                'id_lectura' => $idLectura,
            ...
            CorreccionesService::cambiarEstado($conn, $idCorreccion, 'EJECUTADA', $usuario,
                'Lectura eléctrica corregida con regeneración documental y contable controlada.', $resultado);
    ...
    private static function cargarLectura(PDO $conn, int $idLectura, int $idContrato): ?array
    {
        $stmt = $conn->prepare(
            "SELECT TOP(1) lm.id_lectura,lm.id_medidor,lm.periodo_facturacion,lm.lectura_anterior,
                ...
                pl.valor_kwh,cs.id_cobro_servicio,cs.consumo_cobrado,cs.monto_total monto_cobro,
             ...
             WHERE lm.id_lectura=:lectura ORDER BY dd.id_detalle_documento DESC"
        );
    ...
    private static function cargarSiguiente(PDO $conn, int $idMedidor, string $periodo): ?array
    {
        $stmt = $conn->prepare(
            "SELECT TOP(1) lm.id_lectura,lm.periodo_facturacion,lm.lectura_anterior,lm.lectura_actual,
                lm.consumo_informado,pl.valor_kwh,cs.id_cobro_servicio,cs.consumo_cobrado,
    ...
    private static function revertirContabilidad(PDO $conn, int $idDocumento, int $idCorreccion): bool
    {
        ...
        $reversa = $conn->prepare("EXEC dbo.msp_acc_revertir_origen @tabla_origen=N'msp_documentos_cobro',
            @id_origen=:id,@fecha_reversa=:fecha,@motivo=N'Corrección de lectura eléctrica'");
    ...
    private static function actualizarLecturaYCobro(PDO $conn, array $item, int $idCorreccion): void
    {
        ...
        if ($idCobro > 0) {
            $parametros = json_encode(['servicio'=>'LUZ','valor_kwh'=>(float) $item['valor_kwh'],
                'id_correccion'=>$idCorreccion], JSON_UNESCAPED_UNICODE + JSON_UNESCAPED_SLASHES);
            $updCobro = $conn->prepare("UPDATE dbo.msp_cobros_servicios
                SET consumo_cobrado=:consumo,subtotal_variable=:monto,cargo_fijo=0,monto_total=:total,
                    parametros_snapshot=:parametros,detalle_calculo=N'Corrección selectiva de lectura eléctrica',
            ...
            if ($updCobro->rowCount() !== 1) {
                throw new RuntimeException('El cobro eléctrico asociado cambió o dejó de existir durante la corrección.');
            }
            ...
            if ((int) ($item['id_documento'] ?? 0) > 0 && $updDetalle->rowCount() < 1) {
                throw new RuntimeException('El documento ya no contiene el cobro eléctrico que se intentó corregir.');
            }
        }
        self::impacto($conn, $idCorreccion, 'LECTURA', (int) $item['id_lectura'], 'UPDATE', [
            'lectura_anterior'=>$item['lectura_anterior_original'],'lectura_actual'=>$item['lectura_actual_original'],
            'consumo'=>$item['consumo_anterior'],'monto'=>$item['monto_anterior'],
        ], [
            'lectura_anterior'=>$item['lectura_anterior_nueva'],'lectura_actual'=>$item['lectura_actual_nueva'],
    ...
    private static function actualizarDocumento(PDO $conn, int $idDocumento, float $delta, int $idCorreccion, int $usuario, bool $rehacerAsiento): void
    {
        ...
        DocumentoCobroTrazabilidadService::registrar($conn, $idDocumento, 'RECALCULO', 'SISTEMA', $usuario, [
            'id_correccion'=>$idCorreccion,'tipo'=>'LECTURA_ELECTRICIDAD','delta_documento'=>$delta,
        ]);

...
final class AguaCorreccionService
{
    public static function ejecutar(PDO $conn, array $corr, int $usuario): array
    {
        ...
        foreach (['msp_lecturas_medidores','msp_medidores','msp_contrato_locales','msp_contratos_arriendo',
                  'msp_procesos_cobro_servicio','msp_tipos_servicio','msp_proceso_cobro_agua',
                  'msp_cobros_servicios','msp_documentos_cobro','msp_documentos_cobro_detalle'] as $tabla) {
            if (!msp2TableExists($conn, $tabla)) {
                throw new RuntimeException('Falta la estructura requerida para corregir la lectura de agua.');
            }
        }
        ...
        try {
            ...
            if ((string) ($lectura['codigo_servicio'] ?? '') !== 'AGUA') {
                throw new RuntimeException('La corrección controlada implementada corresponde solamente a agua.');
            }
            if ((int) ($lectura['id_cobro_servicio'] ?? 0) <= 0) {
                throw new RuntimeException('La lectura no tiene un cobro de agua calculado. Vuelve a generar la etapa de agua.');
            }
            ...
            self::validarParametrosAgua($lectura, 'del período');

            $fechaConsumo = (string) ($lectura['fecha_referencia_consumo'] ?? $lectura['periodo_facturacion']);
            $siguiente = self::cargarSiguiente(
                $conn,
                (int) $lectura['id_medidor'],
                $fechaConsumo,
                (int) $lectura['id_lectura']
            );
            ...
            $calculoNuevo = self::calcularCobro($consumoNuevo, $lectura);
            $montoNuevo = (float) $calculoNuevo['monto_total'];
            $afectados = [[
                'tipo' => 'actual',
                ...
                'subtotal_variable_nuevo' => (float) $calculoNuevo['subtotal_variable'],
                'cargo_fijo_nuevo' => (float) $calculoNuevo['cargo_fijo'],
                'servicio_agua_potable' => (float) ($lectura['servicio_agua_potable'] ?? 0),
                'servicio_alcantarillado' => (float) ($lectura['servicio_alcantarillado'] ?? 0),
                'tratamiento_aguas_servidas' => (float) ($lectura['tratamiento_aguas_servidas'] ?? 0),
                'divisor' => (float) ($lectura['divisor'] ?? 0),
                'cargo_fijo_parametro' => (float) ($lectura['cargo_fijo_parametro'] ?? 0),
            ]];
            ...
            if ($propagaSiguiente) {
                ...
                self::validarParametrosAgua($siguiente, 'del período siguiente');
                $consumoSiguiente = round((float) $siguiente['lectura_actual'] - $nuevo, 4);
                $calculoSiguiente = self::calcularCobro($consumoSiguiente, $siguiente);
                $afectados[] = [
                    'tipo' => 'siguiente',
                    ...
                    'monto_nuevo' => (float) $calculoSiguiente['monto_total'],
                    'subtotal_variable_nuevo' => (float) $calculoSiguiente['subtotal_variable'],
                    'cargo_fijo_nuevo' => (float) $calculoSiguiente['cargo_fijo'],
                    'servicio_agua_potable' => (float) ($siguiente['servicio_agua_potable'] ?? 0),
                    'servicio_alcantarillado' => (float) ($siguiente['servicio_alcantarillado'] ?? 0),
                    'tratamiento_aguas_servidas' => (float) ($siguiente['tratamiento_aguas_servidas'] ?? 0),
                    'divisor' => (float) ($siguiente['divisor'] ?? 0),
                    'cargo_fijo_parametro' => (float) ($siguiente['cargo_fijo_parametro'] ?? 0),
                ];
            }
            ...
            $resultado = [
                ...
                'tarifa_variable' => (float) $calculoNuevo['tarifa_variable'],
                'cargo_fijo' => (float) $calculoNuevo['cargo_fijo'],
                'monto_anterior' => (float) ($lectura['monto_cobro'] ?? 0),
                'monto_nuevo' => $montoNuevo,
    ...
    {
        $stmt = $conn->prepare(
            "SELECT TOP(1) lm.id_lectura,lm.id_medidor,lm.periodo_facturacion,lm.fecha_desde_consumo,
                lm.fecha_hasta_consumo,lm.fecha_lectura,
                COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion) fecha_referencia_consumo,
                lm.lectura_anterior,lm.lectura_actual,lm.consumo_informado,m.id_local,m.codigo_medidor,l.cdo_local,
                ...
                pa.divisor,pa.cargo_fijo cargo_fijo_parametro,
                cs.id_cobro_servicio,cs.consumo_cobrado,cs.subtotal_variable,
                cs.cargo_fijo cargo_fijo_cobro,cs.monto_total monto_cobro,
             ...
             WHERE lm.id_lectura=:lectura
               ...
               AND (
                    ...
                    OR (
                        cl.fecha_inicio<=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,EOMONTH(lm.periodo_facturacion))
                        AND (cl.fecha_termino IS NULL OR cl.fecha_termino>=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion))
                        AND ca.fecha_inicio<=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,EOMONTH(lm.periodo_facturacion))
                        AND (ca.fecha_termino_efectiva IS NULL OR ca.fecha_termino_efectiva>=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion))
                    )
               )
    ...
    private static function cargarSiguiente(PDO $conn, int $idMedidor, string $fechaConsumo, int $idLectura): ?array
    {
        $stmt = $conn->prepare(
            "SELECT TOP(1) lm.id_lectura,lm.periodo_facturacion,lm.fecha_desde_consumo,
                lm.fecha_hasta_consumo,lm.fecha_lectura,
                COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion) fecha_referencia_consumo,
                ...
                pa.divisor,pa.cargo_fijo cargo_fijo_parametro,
                cs.id_cobro_servicio,cs.consumo_cobrado,cs.subtotal_variable,
                cs.cargo_fijo cargo_fijo_cobro,
             ...
             OUTER APPLY (
                ...
                WHERE cl.id_local=m.id_local
                  ...
                  AND cl.fecha_inicio<=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,EOMONTH(lm.periodo_facturacion))
                  AND (cl.fecha_termino IS NULL OR cl.fecha_termino>=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion))
                  AND ca.fecha_inicio<=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,EOMONTH(lm.periodo_facturacion))
                  AND (ca.fecha_termino_efectiva IS NULL OR ca.fecha_termino_efectiva>=COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion))
             ...
             WHERE lm.id_medidor=:medidor
               AND (
                    COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion)>:fecha_consumo
                    OR (
                        COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion)=:fecha_consumo_igual
                        AND lm.id_lectura>:lectura
                    )
               )
               AND UPPER(ts.codigo_servicio)=N'AGUA'
             ORDER BY COALESCE(lm.fecha_hasta_consumo,lm.fecha_lectura,lm.periodo_facturacion),
                lm.periodo_facturacion,lm.id_lectura,dd.id_detalle_documento DESC"
        );
        $stmt->execute([
            ':medidor'=>$idMedidor,
            ':fecha_consumo'=>$fechaConsumo,
            ':fecha_consumo_igual'=>$fechaConsumo,
            ':lectura'=>$idLectura,
        ]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        return $row !== false ? $row : null;
    ...
    private static function validarParametrosAgua(array $lectura, string $contexto): void
    {
        $componentes = [
            (float) ($lectura['servicio_agua_potable'] ?? -1),
            (float) ($lectura['servicio_alcantarillado'] ?? -1),
            (float) ($lectura['tratamiento_aguas_servidas'] ?? -1),
            (float) ($lectura['cargo_fijo_parametro'] ?? -1),
        ];
        $divisor = (float) ($lectura['divisor'] ?? 0);
        if ($divisor <= 0 || array_filter($componentes, static fn (float $valor): bool => !is_finite($valor) || $valor < 0) !== []) {
            throw new RuntimeException('Los parámetros de agua ' . $contexto . ' no son válidos.');
        }
    }

    private static function calcularCobro(float $consumo, array $lectura): array
    {
        $divisor = (float) ($lectura['divisor'] ?? 0);
        if ($divisor <= 0) {
            throw new RuntimeException('El divisor del proceso de agua debe ser mayor que cero.');
        }
        $tarifaVariable = (
            (float) ($lectura['servicio_agua_potable'] ?? 0)
            + (float) ($lectura['servicio_alcantarillado'] ?? 0)
            + (float) ($lectura['tratamiento_aguas_servidas'] ?? 0)
        ) / $divisor;
        $subtotalVariable = round($consumo * $tarifaVariable, 2);
        $cargoFijo = round((float) ($lectura['cargo_fijo_parametro'] ?? 0), 2);
        return [
            'tarifa_variable' => $tarifaVariable,
            'subtotal_variable' => $subtotalVariable,
            'cargo_fijo' => $cargoFijo,
            'monto_total' => round($subtotalVariable + $cargoFijo, 2),
        ];
    }
    ...
    private static function revertirContabilidad(PDO $conn, int $idDocumento, int $idCorreccion): bool
    {
        ...
        $reversa = $conn->prepare("EXEC dbo.msp_acc_revertir_origen @tabla_origen=N'msp_documentos_cobro',
            @id_origen=:id,@fecha_reversa=:fecha,@motivo=N'Corrección de lectura de agua'");
    ...
    private static function actualizarLecturaYCobro(PDO $conn, array $item, int $idCorreccion): void
    {
        ...
        if ($idCobro > 0) {
            $parametros = json_encode([
                'servicio'=>'AGUA',
                'servicio_agua_potable'=>(float) $item['servicio_agua_potable'],
                'servicio_alcantarillado'=>(float) $item['servicio_alcantarillado'],
                'tratamiento_aguas_servidas'=>(float) $item['tratamiento_aguas_servidas'],
                'divisor'=>(float) $item['divisor'],
                'cargo_fijo'=>(float) $item['cargo_fijo_parametro'],
                'id_correccion'=>$idCorreccion,
            ], JSON_UNESCAPED_UNICODE + JSON_UNESCAPED_SLASHES);
            $updCobro = $conn->prepare("UPDATE dbo.msp_cobros_servicios
                SET consumo_cobrado=:consumo,subtotal_variable=:subtotal,cargo_fijo=:cargo_fijo,monto_total=:total,
                    formula_version=N'V3_AGUA_EXCEL_2604',
                    parametros_snapshot=:parametros,
                    detalle_calculo=N'AGUA Excel: cargo fijo completo por medidor + consumo * ((SAP + SAL + TAS)/divisor)',
                    fecha_calculo=SYSDATETIME() WHERE id_cobro_servicio=:id");
            $updCobro->execute([
                ':consumo'=>(float) $item['consumo_nuevo'],
                ':subtotal'=>(float) $item['subtotal_variable_nuevo'],
                ':cargo_fijo'=>(float) $item['cargo_fijo_nuevo'],
                ':total'=>(float) $item['monto_nuevo'],
                ':parametros'=>$parametros,
                ':id'=>$idCobro,
            ]);
            if ($updCobro->rowCount() !== 1) {
                throw new RuntimeException('El cobro de agua asociado cambió o dejó de existir durante la corrección.');
            }
            $consumo = (float) $item['consumo_nuevo'];
            ...
            if ((int) ($item['id_documento'] ?? 0) > 0 && $updDetalle->rowCount() < 1) {
                throw new RuntimeException('El documento ya no contiene el cobro de agua que se intentó corregir.');
            }
        }
        self::impacto($conn, $idCorreccion, 'LECTURA', (int) $item['id_lectura'], 'UPDATE', [
            'servicio'=>'AGUA','lectura_anterior'=>$item['lectura_anterior_original'],'lectura_actual'=>$item['lectura_actual_original'],
            'consumo'=>$item['consumo_anterior'],'monto'=>$item['monto_anterior'],
        ], [
            'servicio'=>'AGUA','lectura_anterior'=>$item['lectura_anterior_nueva'],'lectura_actual'=>$item['lectura_actual_nueva'],
            'consumo'=>$item['consumo_nuevo'],'monto'=>$item['monto_nuevo'],
            'servicio_agua_potable'=>$item['servicio_agua_potable'],
            'servicio_alcantarillado'=>$item['servicio_alcantarillado'],
            'tratamiento_aguas_servidas'=>$item['tratamiento_aguas_servidas'],
            'divisor'=>$item['divisor'],'cargo_fijo'=>$item['cargo_fijo_parametro'],
        ], true);
    }

...
if (!function_exists('msp2MontoClpEscape')) {
    function msp2MontoClpEscape(string $value): string
    {
...
if (!function_exists('msp2MontoClpFormatValue')) {
    function msp2MontoClpFormatValue(string $value, int $decimals = 2): string
    {
...
if (!function_exists('msp2RenderMontoClpAssets')) {
    function msp2RenderMontoClpAssets(): void
    {
...
if (!function_exists('msp2RenderMontoClpField')) {
    /**
     * @param array{
     ...
     * } $options
     */
    function msp2RenderMontoClpField(array $options = []): void
    {
        $wrapperClass = trim((string) ($options['wrapper_class'] ?? 'col-12 col-lg-3'));
        $id = trim((string) ($options['id'] ?? 'monto_cargo_input'));
        $name = trim((string) ($options['name'] ?? 'monto_cargo'));
        ...
        $renderValue = $formatInitial ? msp2MontoClpFormatValue($value, $decimals) : $value;
        ?>
        <div class="<?php echo msp2MontoClpEscape($wrapperClass); ?>">
            <label class="form-label msp-monto-label" for="<?php echo msp2MontoClpEscape($id); ?>">
                <i class="bi bi-cash-coin" aria-hidden="true"></i><?php echo msp2MontoClpEscape($label); ?>
            </label>
            <div class="input-group msp-monto-group">
                <span class="input-group-text">$</span>
                <input
                    type="text"
                    class="form-control msp-monto-input js-monto-clp"
                    id="<?php echo msp2MontoClpEscape($id); ?>"
                    name="<?php echo msp2MontoClpEscape($name); ?>"
                    value="<?php echo msp2MontoClpEscape($renderValue); ?>"
                    placeholder="<?php echo msp2MontoClpEscape($placeholder); ?>"
                    inputmode="decimal"
                    autocomplete="<?php echo msp2MontoClpEscape($autocomplete); ?>"
                    <?php echo $required ? 'required' : ''; ?>>
            </div>
            <?php if ($hint !== ''): ?>
                <div class="ce-subtle mt-1"><?php echo msp2MontoClpEscape($hint); ?></div>
            <?php endif; ?>
        </div>
        <?php
    }

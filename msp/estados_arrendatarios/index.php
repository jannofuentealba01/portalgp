 ...
 */
function msp2EstadosArrendatariosBuildPaginationItems(int $paginaActual, int $totalPaginas): array
{
    if ($totalPaginas <= 1) {
        return [];
    }
...
try {
    $tablaExiste = msp2TableExists($conn, 'msp_estado_arrendatarios');
    if (!$tablaExiste) {
        $loadError = 'La tabla `msp_estado_arrendatarios` no existe todavía. Ejecuta `msp/msp_a1.sql` antes de continuar.';
    }
} catch (PDOException $exception) {
    $tablaExiste = false;
    $loadError = 'No fue posible validar la estructura base de estados de arrendatarios.';
}
...
if ($tablaExiste) {
    try {
        ...
        if ($filtroTexto !== '') {
            $search = msp2BuildSearchCondition($filtroTexto, ['desc_estado', 'id_estado_arrendatario'], 'estado_arr_buscar', 'id_estado_arrendatario');
        ...
        $countStmt = $conn->prepare("SELECT COUNT(*) FROM dbo.msp_estado_arrendatarios WHERE $whereClause");
        ...
        $stmt = $conn->prepare(
            "SELECT id_estado_arrendatario, desc_estado
        ...
        $loadError = pgpPublicException($exception, 'msp.estados_arrendatarios.index', 'No fue posible cargar los estados de arrendatarios.');
    }
}

if ($tablaExiste) {
    $paginationItems = msp2EstadosArrendatariosBuildPaginationItems($paginaActual, $totalPaginas);
}

$queryBase = $_GET;
unset($queryBase['pagina']);

function buildMsp2EstadosArrendatariosQuery(array $base, array $override = []): string
{
...
<head>
    ...
    <title>MSP | Estados de Arrendatarios</title>
...
<main class="gp-main d-flex align-items-center justify-content-center p-4">
    <div class="box-container-wide">
        ...
        gpRenderSectionHeader([
            ...
            'title' => 'Estados de Arrendatarios',
            'description' => 'Catálogo base para estado operativo de arrendatarios.',
            'back_url' => msp2Url('catalogo_menu.php'),
            'back_label' => 'Volver a catálogos',
            'help_text' => 'Administra el catálogo de estados operativos de arrendatarios que se usa en los flujos del módulo MSP.',
            'help_aria_label' => 'Información de la sección Estados de Arrendatarios',
        ]);
        ?>
        ...
        <?php else: ?>
            <form method="get" class="row g-2 mb-3 align-items-end">
                ...
                <div class="col-12 col-md-2 d-flex gap-2">
                    ...
                    <?php if ($filtroTexto !== ''): ?><a class="btn btn-outline-secondary" href="<?php echo msp2Escape(msp2Url('estados_arrendatarios/index.php')); ?>" title="Limpiar" aria-label="Limpiar"><i class="bi bi-x-lg"></i></a><?php endif; ?>
                </div>
            </form>
            ...
            gpRenderCrudTable([
                                    ...
                                    'items' => [
                                        [
                                            ...
                                            'attrs' => [
                                                ...
                                                'data-id' => (string) ((int) ($estado['id_estado_arrendatario'] ?? 0)),
                                        ...
                                        [
                                            ...
                                            'form_attrs' => [
                                                'method' => 'post',
                                                'action' => msp2Url('estados_arrendatarios/eliminar.php'),
                                                'data-confirm-message' => '¿Eliminar el estado "' . (string) ($estado['desc_estado'] ?? '') . '"?',
                                                'data-confirm-title' => 'Confirmar eliminación',
                                                'data-confirm-variant' => 'danger',
                                            ],
                                            'fields' => [
                                                'id_estado_arrendatario' => (string) ((int) ($estado['id_estado_arrendatario'] ?? 0)),
                                            ],
                ...
                'pagination' => [
                    ...
                    'aria_label' => 'Paginación de estados de arrendatarios',
                    'build_url' => static fn (int $page): string => buildMsp2EstadosArrendatariosQuery($queryBase, ['pagina' => $page]),
                ],
            ]);
            ?>
        <?php endif; ?>
...
<div class="modal fade" id="modalCrearEstado" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" method="post" action="<?php echo msp2Escape(msp2Url('estados_arrendatarios/guardar.php')); ?>">
...
<div class="modal fade" id="modalEditarEstado" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" method="post" action="<?php echo msp2Escape(msp2Url('estados_arrendatarios/guardar.php')); ?>">
            ...
            <div class="modal-body">
                <input type="hidden" name="id_estado_arrendatario" id="edit_id_estado_arrendatario">
                <div class="mb-3">
...
(() => {
    document.querySelectorAll('.js-edit-estado').forEach((button) => {
        button.addEventListener('click', () => {
            document.getElementById('edit_id_estado_arrendatario').value = button.dataset.id || '';

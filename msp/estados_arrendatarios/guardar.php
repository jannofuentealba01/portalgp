...
try {
    ...
    if ($idEstadoArrendatario !== false && $idEstadoArrendatario !== null) {
        $existsStmt = $conn->prepare('SELECT COUNT(*) FROM dbo.msp_estado_arrendatarios WHERE id_estado_arrendatario = :id_estado_arrendatario');
        $existsStmt->bindValue(':id_estado_arrendatario', $idEstadoArrendatario, PDO::PARAM_INT);
        $existsStmt->execute();
        $existe = (int) $existsStmt->fetchColumn() > 0;
    }

    $dupStmt = $conn->prepare(
        'SELECT COUNT(*) FROM dbo.msp_estado_arrendatarios WHERE desc_estado = :desc_estado' .
        ($existe ? ' AND id_estado_arrendatario <> :id_estado_arrendatario' : '')
    );
    $dupStmt->bindValue(':desc_estado', $descEstado, PDO::PARAM_STR);
    if ($existe) {
        $dupStmt->bindValue(':id_estado_arrendatario', $idEstadoArrendatario, PDO::PARAM_INT);
    }
    $dupStmt->execute();
    ...
    if ($existe) {
        $stmt = $conn->prepare('UPDATE dbo.msp_estado_arrendatarios SET desc_estado = :desc_estado WHERE id_estado_arrendatario = :id_estado_arrendatario');
        ...
        $stmt = $conn->prepare('INSERT INTO dbo.msp_estado_arrendatarios (desc_estado) VALUES (:desc_estado)');
        $stmt->bindValue(':desc_estado', $descEstado, PDO::PARAM_STR);
        $stmt->execute();

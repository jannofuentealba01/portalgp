...
$contratoGuardar=(string)file_get_contents(dirname(__DIR__).'/msp/contratos/guardar.php');
$check('Contrato sincroniza ocupación antes de insertar garantía',function()use($contratoGuardar):bool{$ocupacion=strpos($contratoGuardar,'$stmtInsertOcupacion->execute();');$garantia=strrpos($contratoGuardar,'$stmtInsertGarantia->execute();');return $ocupacion!==false&&$garantia!==false&&$ocupacion<$garantia;});

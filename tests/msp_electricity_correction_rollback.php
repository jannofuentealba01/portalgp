...
/**
 * Prueba integral reversible de la corrección eléctrica controlada.
 * Toda escritura se ejecuta dentro de una transacción que siempre termina en rollback.
 */

function electricityTestAssert(bool $condition, string $message): void
{
    if (!$condition) {
...
electricityTestAssert(
    $testDatabase !== ''
        && strtoupper($testDatabase) !== 'PORTALGP'
        && preg_match('/^[A-Za-z0-9_]+$/', $testDatabase) === 1,
    'La prueba solo puede ejecutarse contra una base aislada indicada en MSP_TEST_DB.'
);
...
$readingId = isset($argv[1]) ? (int) $argv[1] : 1123;
electricityTestAssert($readingId > 0, 'Debes indicar un ID de lectura válido.');
...
electricityTestAssert(is_array($before), 'No se encontró la lectura de prueba.');
electricityTestAssert((int) ($before['id_documento_cobro'] ?? 0) > 0, 'La prueba requiere una lectura documentada.');
electricityTestAssert((int) ($before['id_cobro_servicio'] ?? 0) > 0, 'La prueba requiere un cobro eléctrico calculado.');
electricityTestAssert((int) ($before['asientos_activos'] ?? 0) > 0, 'La prueba requiere un asiento contable activo.');

$previous = (float) $before['lectura_anterior'];
$current = (float) $before['lectura_actual'];
$newReading = round($current - 1, 4);
electricityTestAssert($newReading >= $previous, 'La lectura no tiene margen suficiente para una prueba reversible.');

$userStmt = $conn->query("SELECT TOP(1) id FROM dbo.cr_usuarios WHERE UserName=N'admin_2'");
$userId = (int) ($userStmt->fetchColumn() ?: 0);
electricityTestAssert($userId > 0, 'No se encontró el usuario admin_2 para atribuir la prueba.');

$conn->beginTransaction();
try {
    $correctionId = CorreccionesService::crearSolicitud($conn, [
        'tipo_correccion' => 'LECTURA',
        'modulo_origen' => 'tests/msp_electricity_correction_rollback.php',
        ...
        'motivo' => 'Prueba automatizada reversible de corrección eléctrica',
        'resultado_analisis' => [
            'registro_exacto' => ['servicio' => 'LUZ', 'id_lectura' => $readingId],
    ...
    electricityTestAssert(is_array($during), 'No fue posible releer el resultado transaccional.');
    electricityTestAssert(abs((float) $during['lectura_actual'] - $newReading) <= 0.0001, 'La lectura no fue actualizada.');
    electricityTestAssert((float) $during['monto_documento'] < (float) $before['monto_documento'], 'El documento no fue recalculado.');
    electricityTestAssert((int) $during['asientos_activos'] === 1, 'La contabilidad no quedó con un único asiento activo.');
    ...
    electricityTestAssert($statusStmt->fetchColumn() === 'EJECUTADA', 'La corrección no terminó en estado EJECUTADA.');
    ...
    electricityTestAssert((int) $impactStmt->fetchColumn() >= 2, 'No se registraron todos los impactos esperados.');
    ...
    electricityTestAssert((float) $balanceStmt->fetchColumn() <= 0.01, 'El asiento regenerado no quedó cuadrado.');

    $conn->rollBack();

    $snapshotStmt->execute([':lectura' => $readingId]);
    $after = $snapshotStmt->fetch(PDO::FETCH_ASSOC);
    electricityTestAssert(is_array($after), 'No fue posible comprobar el rollback.');
    electricityTestAssert(abs((float) $after['lectura_actual'] - $current) <= 0.0001, 'El rollback no restauró la lectura.');
    electricityTestAssert(
    ...
    echo "OK: corrección eléctrica, documento, trazabilidad y contabilidad validados con rollback.\n";
} catch (Throwable $e) {
